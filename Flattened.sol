...
// File contracts/InkomsPunksDNA.sol


pragma solidity ^0.8.0;

contract InkomsPunksDNA {
    ...
    // Get attributes
    uint256 constant ADN_SECTION_SIZE = 2;
    ...
    function _getDNASection (uint256 _dna, uint256 _rightDiscard)
        ...
        returns (uint256)
    {
        return uint256((_dna % (1 * 10 ** (_rightDiscard + ADN_SECTION_SIZE))) / (1 * 10 ** _rightDiscard));
    ...
    function getClotheColor(uint256 _dna)
        public
        view
        returns (string memory) {
...
// File contracts/InkomsPunks.sol


pragma solidity ^0.8.0;
contract InkomsPunks is ERC721, ERC721Enumerable, InkomsPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("InkomsPunks", "IPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No Inkoms Punks left :(");
    ...
    function _paramsURI(uint256 _dna) internal view
        returns(string memory) {
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "InkomsPunks #',
                tokenId.toString(),
                '", "description": "Inkoms Punks are randomized Avataaars stored on chain to teach DApp development on Inkoms", "image": "',
                image,
                '"}'
            )
        );
